...
import static camp.model.ScoreManger.*;
import static camp.model.StudentManager.*;
...
//        studentInformation = new HashMap<>();
    ...
    private static void displayStudentView() throws InterruptedException  {
        ...
        while (flag) {
            ...
            switch (input) {
                case 1 -> createStudent(subjectStore, studentStore); // 수강생 등록
                case 2 -> inquireStudent(studentStore); // 수강생 목록 조회
                case 3 -> studentInformation(studentStore); // 수강생 상세정보 조회
                case 4 -> editStudentInformation(studentStore, subjectStore); // 수강생 정보 수정
                case 5 -> studentRemove(studentStore, scoreStore); // 수강생 삭제
                case 6 -> studentBYColor(studentStore);// 수강생 상태별 조회
    ...
    private static void displayScoreView() throws InterruptedException  {
        ...
        while (flag) {
            ...
            switch (input) {
                case 1 -> createScore(subjectStore, scoreStore, studentStore); // 수강생의 과목별 시험 회차 및 점수 등록
                case 2 -> updateRoundScoreBySubject(scoreStore, subjectStore, studentStore); // 수강생의 과목별 회차 점수 수정
                case 3 -> inquireRoundGradeBySubject(scoreStore, studentStore, subjectStore); // 수강생의 특정 과목 회차별 등급 조회
                case 4 -> average(studentStore, scoreStore);
                case 5 -> studentMandatoryAverage(subjectStore, studentStore, scoreStore);
